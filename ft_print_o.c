...
static void	print_if_zero_flag(struct s_specifier *flags, int *i,
								  long num, char *b)
{
	*i += print_char_len_times('0', flags->width - len_nbr_base(num, 8));
	*i += ft_putnbr_base(num, 8, b);
}
...
*i, long num, char *b)
{
	...
	k = 0;
	if (flags->accur >= len_nbr_base(num, 8))
	{
		*i += print_char_len_times('0', flags->accur - len_nbr_base(num, 8));
		k = (flags->accur - len_nbr_base(num, 8));
	}
	*i += ft_putnbr_base(num, 8, b);
	*i += print_char_len_times(' ', flags->width - len_nbr_base(num, 8) - k);
}
...
*i, long num, char *b)
{
	...
	{
		*i += print_char_len_times(' ', flags->width - len_nbr_base(num, 8));
	}
	*i += ft_putnbr_base(num, 8, b);
}
